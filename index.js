...
(function () {
  ...
  function currentPersonaKeyDebug() {
    ...
    // 方式二：从 DOM 里找当前选中的头像（兼容多种可能的 class / 结构写法）
    try {
      const $selected = $(
        [
      ...
      if (match && match[1]) {
        debug.method = 'DOM选中头像';
        debug.raw = src || '(空src)';
        return { key: 'avatar:' + decodeURIComponent(match[1]), debug };
      }
      debug.raw = 'DOM未匹配到selected头像，src=' + (src || '(未找到元素)');
    } catch (e) {
  ...
  function buildPopupHtml(debugInfo) {
    return `
      <div class="upd-wrap">
        <h3 class="upd-title">
          ✙ 人设差分 ✙
        </h3>
        <div class="upd-debug-bar">
          🔧调试信息（确认无误后可删除）<br/>
          识别方式：${escapeHtml(debugInfo.method || '(无)')}<br/>
          原始值：${escapeHtml(debugInfo.raw || '(无)')}<br/>
          最终Key：${escapeHtml(debugInfo.key || '(无)')}
        </div>
        <div id="upd_body"></div>
      </div>
    `;
  ...
  function renderDetailPage($body, key) {
    ...
    const $html = $(`
      <div class="upd-detail-card">
        ...
        <div class="upd-bottom-actions">
          ...
          <div class="menu_button" id="upd_close_popup"><span class="fa-solid fa-xmark"></span> 关闭</div>
        </div>
      </div>
    `);
    ...
    $html.find('#upd_close_popup').on('click', () => {
      const $okBtn = $body
        .closest('.popup')
        .find('.popup-button-ok, .popup_ok, #dialogue_popup_ok')
        .first();
      if ($okBtn.length) {
        $okBtn.trigger('click');
      }
  ...
  async function openPopup() {
    const { key, debug } = currentPersonaKeyDebug();
    popupState = { selectedId: null, editing: false };

    const $el = $(buildPopupHtml({ ...debug, key }));
    render($el, key);
