...
(function () {
  ...
  function currentPersonaKey() {
    try {
      const $selected = $(
        '#user_avatar_block .avatar-container.selected img, #user_avatar_block img.selected'
      ).first();
      ...
      const match =
        ...
        src.match(/avatar=([^&]+)/i);
      if (match && match[1]) return 'avatar:' + decodeURIComponent(match[1]);
    } catch {}
    const name = ($('#persona_name').val() || '').toString().trim();
    return name ? 'name:' + name : 'default';
  }

  function currentPersonaDisplayName() {
    const name = ($('#persona_name').val() || '').toString().trim();
    return name || '(未命名 User)';
  }
  ...
  function buildPopupHtml() {
    return `
      <div class="upd-wrap">
        <h3 class="upd-title">
          ✙ 人设差分 ✙
          <span class="upd-title-sub">· ${escapeHtml(currentPersonaDisplayName())}</span>
        </h3>
        <div id="upd_body"></div>
      </div>
    `;
  ...
  function renderDetailPage($body, key) {
    ...
    const $back = $('<div class="upd-back"><span class="fa-solid fa-arrow-left"></span> 返回列表</div>');
    $back.on('click', () => {
      popupState.selectedId = null;
      render($body.closest('.upd-wrap'), key);
    });
  ...
  function renderEditPage($body, key) {
    ...
    const $back = $('<div class="upd-back"><span class="fa-solid fa-arrow-left"></span> 取消并返回</div>');
    $back.on('click', () => {
      popupState.editing = false;
    ...
    const $form = $(`
      <div class="upd-form">
        <div class="upd-field">
          ...
          <input class="text_pole upd-name-input" id="upd_f_name" placeholder="差分名字（应用后会覆盖 persona 名字）" value="${escapeHtml(v.name)}" />
        </div>
        <div class="upd-field">
        ...
        <div class="upd-field">
          ...
          <input class="text_pole" id="upd_f_note" placeholder="可选的简短备注" value="${escapeHtml(v.note)}" />
        </div>
        <div class="upd-field">
        ...
        <div class="upd-bottom-actions">
          ...
          <div class="menu_button" id="upd_f_cancel">取消</div>
        </div>
      </div>
    `);
