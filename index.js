...
(function () {
  ...
  function currentPersonaKey() {
    ...
    // （不同版本变量名可能不同，这里做多重兜底，只要拿到非空值就当作唯一 key）
    try {
      const candidates = [
        context.userAvatar,
        context.user_avatar,
    ...
    // 方式三：兜底用当前 persona 名字输入框的值（同名 persona 仍会冲突，但优于全部混在一起）
  ...
  async function openPopup() {
    ...
    console.info('[st-user-persona-diff] 当前 persona key:', key);
